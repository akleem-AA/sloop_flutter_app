  ...
  Future<void> getCartDataOnline() async {
    if (Get.find<SplashController>().module != null ||
      ...
      if (response.statusCode == 200) {
        ...
        ///to store data from carts key
        print(
            "------------------------------------------------------response is ${response.body['related_products']} ");
        print(response.body);
        response.body['carts'].forEach(
            (cart) => _onlineCartList.add(OnlineCartModel.fromJson(cart)));

        ///to store data from relatedItem key
        relatedItemList.clear();
        response.body['related_products']
            .forEach((item) => relatedItemList?.add(Item.fromJson(item)));
        //
        _cartList.addAll(CartHelper.formatOnlineCartToLocalCart(
            onlineCartModel: _onlineCartList));
