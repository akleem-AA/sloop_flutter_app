...
class _ItemWidgetState extends State<ItemWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isStore) {
      discount =
          widget.store!.discount != null ? widget.store!.discount!.discount : 0;
      discountType = widget.store!.discount != null
          ? widget.store!.discount!.discountType
          : 'percent';
          ...
          if (widget.store != null) {
            ...
            Get.toNamed(
              RouteHelper.getStoreRoute(
                  id: widget.store!.id,
                  page: widget.isFeatured ? 'module' : 'item'),
              arguments: StoreScreen(
                  store: widget.store, fromModule: widget.isFeatured),
            );
          }
        } else {
          if (widget.isFeatured &&
            ...
            children: [
              Container(
                padding: ResponsiveHelper.isDesktop(context)
                    ? EdgeInsets.all(widget.fromCartSuggestion
                        ...
                        child: Row(children: [
                          Stack(children: [
                            ClipRRect(
                              ...
                              child: CustomImage(
                                image:
                                    '${widget.isCampaign ? baseUrls!.campaignImageUrl : widget.isStore ? baseUrls!.storeImageUrl : baseUrls!.itemImageUrl}'
                                ...
                                height: widget.imageHeight ??
                                    (desktop
                                        ...
                                        : widget.length == null
                                            ...
                                            : 65),
                                width:
                                    widget.imageWidth ?? (desktop ? 120 : 80),
                                fit: BoxFit.cover,
                              ),
                            ),
                            (widget.isStore || widget.isCornerTag!)
                                ? DiscountTag(
                                    discount: discount,
                                    discountType: discountType,
                                    freeDelivery: widget.isStore
                            ...
                            !widget.isStore
                                ? OrganicTag(
                                    item: widget.item!, placeInImage: true)
                                : const SizedBox(),
                            isAvailable
                                ? const SizedBox()
              ...
              (!widget.isStore && widget.isCornerTag! == false)
                  ? Positioned(
                      right: ltr ? 0 : null,
                      left: ltr ? null : 0,
                      child: CornerDiscountTag(
                        ...
                        discountType: discountType,
                        freeDelivery:
                            widget.isStore ? widget.store!.freeDelivery : false,
                      ))
                  : const SizedBox(),
            ],
          )),
